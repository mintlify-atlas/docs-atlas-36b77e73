...
description: "Create, list, query, and manage your NotebookLM notebooks from the terminal using the nlm notebook subcommands."
---

Notebooks are the top-level containers in NotebookLM — everything else (sources, studio artifacts, research tasks) lives inside one. The `nlm notebook` commands let you manage the full notebook lifecycle from creation through deletion, including an AI-powered `describe` command that summarizes what is inside a notebook and suggests follow-up topics.
...
```bash
nlm notebook list
nlm create notebook list   # verb-first alias
```
...
Ask the AI to summarise the notebook and suggest relevant topics to explore. This calls the NotebookLM AI layer — not a local operation — so it requires a valid session.
