...
description: "Connect any MCP-compatible AI assistant to NotebookLM. The MCP server exposes 35 tools for notebooks, sources, research, studio content, and more."
---
...
<Warning>
  **Context window notice:** The MCP server registers **35 tools** with your AI assistant. Each tool consumes a small amount of context on every request. Disable the server when you are not actively using NotebookLM to preserve context budget. In Claude Code, type `@notebooklm-mcp` to toggle it on or off.
</Warning>
...
The server exposes **35 tools** organized into the following categories. See the [Tools Reference](/mcp/tools-reference) for full parameter documentation and examples.

| Category | Tools | What it covers |
|---|---|---|
| **Notebooks** | 6 | Create, list, get, describe, rename, delete |
| **Sources** | 6 | Add (URL/text/file/Drive), list Drive, sync Drive, delete, describe, get content |
| **Querying** | 2 | Ask AI about notebook sources, configure chat behavior |
...
| **Sharing** | 3 | Get sharing status, enable/disable public link, invite collaborators |
